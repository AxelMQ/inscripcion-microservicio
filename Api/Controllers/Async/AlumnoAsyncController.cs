...
public class AlumnoAsyncController : ControllerBase
{
    ...
    public AlumnoAsyncController(HotHangfireServerManager queues, IJobResultRepository results)
    {
        _queues  = queues;
        _results = results;
    }
    ...
    [HttpPost]
    public Task<IActionResult> Create([FromRoute] string queue, [FromBody] AlumnoCreateDto dto)
    ...
    [HttpPut("{id:int}")]
    public Task<IActionResult> Update([FromRoute] string queue, [FromRoute] int id, [FromBody] AlumnoUpdateDto dto)
